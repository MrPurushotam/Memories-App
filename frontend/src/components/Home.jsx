...
const Home = () => {
  ...
  const fetched= useRef({posts:false})
  ...
  useEffect(()=>{
    !fetched.current.posts && getUserPosts()
  },[fetched])
  ...
  return (
    <div className="w-[100vw] h-[100vh] overflow-hidden">
      <div className="w-full flex justify-between border-2 border-black py-3 px-5 items-center ">
        <button className="border-2 border-black rounded-md px-3 py-2 font-semibold shadow-md" onClick={()=>navigate("/dashboard")} >Dashboard</button>
        <h2 className="text-xl font-semibold select-none cursor-default">Memories</h2>
        <button className={`${loginState?"bg-red-500 shadow-red-300":"bg-black bg-opacity-80 shadow-black"} text-lg text-white rounded-md px-3 py-2 shadow-md `}
        onClick={async()=>{
          if(!loginState){
            navigate("/login")
            return
